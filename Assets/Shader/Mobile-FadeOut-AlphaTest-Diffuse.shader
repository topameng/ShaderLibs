Shader "Topameng/FadeOut/Transparent/Cutout/Diffuse" 
{
	Properties 
	{
		...
		_Invert("Invert", Range(0.01, 1)) = 0.01
	}
	...
	SubShader 
	{
		...
		Pass 
		{
			...
			{
				fixed tex = tex2D(_MainTex, i.uv).a;	
				fixed noise = tex2D(_NoiseTex, i.uv * 2).r;																	
				fixed alpha = min(tex, noise);
				clip(alpha - i.threshold);
